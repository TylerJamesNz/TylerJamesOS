...
**Account.** A money container Tyler owns or controls — either a Deposit Account or an Investment Account. Each Account holds a Categorical Slot for its colour, which lets it slice consistently across any chart that breaks down a money-flow by Account.
...
**Available.** UI-only metric label for spendable money — by definition the part of total balances that is not held in Investment Accounts. The total of the latest Balance Snapshot across all Deposit Accounts, optionally adjusted for transactions since that snapshot. Not an entity name; never appears in table or column names. (Renamed from "Liquid": the term names a category of money, decoupled from any specific chart binding.)

**Net Worth.** The sum of the latest Balance Snapshot across all Accounts (Deposit + Investment).

**Saved.** UI-only metric label for "Income − Expense across the active period." Naming preserves the existing `stat-saved` / `savedForPeriod()` convention; the term does not imply tax treatment or any deliberate set-aside, just the period's gap. Not an entity name.
