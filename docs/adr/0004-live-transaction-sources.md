...
**Status:** Accepted, 2026-05-11. Step 4 superseded by ADR 0005 (also 2026-05-11) after a verification pass disproved the load-bearing assumption that ANZ AU personal accounts can be configured for per-transaction email alerts. Steps 1, 2, 3 (with 2026-05-12 V1 amendment), 5, 6 (with 2026-05-12 V1 amendment) remain. T1d's shape also refined: Akahu Personal App pattern (App Token + User Token, no end-user OAuth round-trip) via the npm `akahu` SDK (v2.5.0), against the post-CDR-transition endpoint surface (Akahu handles the switch 2026-05-31).
...
3. **NZ live: Akahu via Personal App.** ~~Tyler registers an Akahu developer app once. TJOS settings has a "Connect Akahu" button that runs the consent flow. Access + refresh tokens land in Supabase Vault.~~ **V1 amendment (2026-05-12):** Tyler registers a Personal App at `my.akahu.nz` once; the pre-minted App Token and User Token are set as Supabase Edge Function env vars (`AKAHU_APP_TOKEN`, `AKAHU_USER_TOKEN`). No end-user OAuth round-trip is needed because the app and the end user are the same person (single-user TJOS). A Supabase Edge Function (`akahu-sync`) polls daily plus on demand.
...
6. **Two new tables: `external_integrations` (OAuth tokens, Vault-encrypted) and `live_sync_runs` (append-only log, powers the review queue).** Schema additions on `transactions` and `accounts` documented in `_project/data-model.md`. **V1 amendment (2026-05-12):** `external_integrations` ships in Phase J but is unused in V1 because Akahu uses Personal App env-var tokens (see step 3). The table is reserved for rotating-OAuth providers (e.g. future Gmail re-enablement) or multi-user. `live_sync_runs` ships in T1d with `(id, user_id, provider, started_at, finished_at, status, inserted_count, error_message, trigger)`.
